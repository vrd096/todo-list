import { useEffect, useState, KeyboardEvent, useRef, forwardRef } from 'react';
...
const TaskDetails = () => {
  ...
  const clearTaskReminder = async (task: Todo) => {
    if (task?.reminder != '') {
      if (task) {
        const eventId = await deleteEventGoogleCalendar(task);
  ...
  const CustomInputDeadline = forwardRef<HTMLButtonElement, {}>(({ onClick }: any, ref) =>
    todo?.deadline !== '' ? (
      <div className={styles.deadlineWrapper}>
        <div className={classNames(styles.deadlineTime, styles.deadlineTimeCustom)}>
          <button className={styles.deadlineButton} onClick={onClick} ref={ref}>
      ...
      <button
        className={classNames(styles.statusTasksButton, styles.deadlineTimeCustom)}
        onClick={onClick}
        ref={ref}>
        <svg
          className={styles.detailsIcon}
    ...
    todo?.reminder !== '' ? (
      <div className={styles.remiderWrapper}>
        <div className={styles.reminderTime}>
          <button onClick={onClick} ref={ref} className={styles.reminderButton}>
            <svg
              className={styles.detailsIcon}
              width="18px"
              height="18px"
      ...
      <button onClick={onClick} ref={ref} className={styles.statusTasksButton}>
        <svg
          className={styles.detailsIcon}
          width="18px"
          height="18px"
        ...
        <div className={styles.managingStatusTasks}>
          ...
          {window.innerWidth < 1024 ? (
            ...
            <DatePicker
              ...
              onCalendarClose={handleCloseDeadlineCalendar}
              open={true}
          ...
          {window.innerWidth < 1024 ? (
            ...
            <DatePicker
              ...
              portalId="root-portal"
              shouldCloseOnSelect={true}
              onCalendarClose={handleCloseReminderCalendar}
              open={true}
              // showTimeInput
